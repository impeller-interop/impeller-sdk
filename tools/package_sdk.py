...
DEFAULT_ZIG = "zig"
DEFAULT_RELEASE_REPOSITORY = "impeller-interop/impeller-sdk"
PLATFORM_DEP_NAMES = {
...
def write_package_zon(
    root: Path, name: str, version: str, fingerprint: str | None
) -> None:
    ...
    with output.open("wb") as file:
        with gzip.GzipFile(filename="", mode="wb", fileobj=file, mtime=0) as gzip_file:
            with tarfile.open(
                ...
                archive.add(
                    package_root, arcname=package_name, filter=normalize_tar_member
                )
...
def release_asset_exists(release_base_url: str) -> bool:
    return fetch_sdk.artifact_exists(
        f"{release_base_url}/{PLATFORM_ASSET_NAMES['linux-x64']}"
    )
...
def update_zon(path: Path, snippet: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            f"{path} does not contain generated SDK dependency markers"
        ) from error
    if start >= end:
...
def resolve_engine_sha(
    ...
    release = fetch_sdk.latest_release(channel, fetch_sdk.DEFAULT_RELEASES_URL)
    framework_sha_value = release.get("hash")
    if not isinstance(framework_sha_value, str):
        raise RuntimeError(
            f"latest {channel} release does not contain a framework hash"
        )
    engine_sha = fetch_sdk.engine_hash_for_flutter(
        framework_sha_value, fetch_sdk.DEFAULT_FLUTTER_REPO_RAW_URL
    )
...
def package_platform(
    *,
    ...
    archive_url = (
        f"{fetch_sdk.DEFAULT_BASE_URL}/{engine_sha}/{platform}/impeller_sdk.zip"
    )
    ...
    write_package_zon(package_root, dep_name, DEFAULT_VERSION, None)
    create_tarball(package_root, dep_name, tarball)
    fingerprint = suggested_fingerprint(DEFAULT_ZIG, tarball)
    if fingerprint:
        write_package_zon(package_root, dep_name, DEFAULT_VERSION, fingerprint)
        create_tarball(package_root, dep_name, tarball)
    package_hash = zig_fetch_hash(DEFAULT_ZIG, tarball)
    print("OK")
...
def main() -> int:
    ...
    parser.add_argument(
        "--sha",
        help="Flutter engine commit hash. Defaults to the latest release for --channel.",
    )
    parser.add_argument(
        "--channel",
        choices=fetch_sdk.RELEASE_CHANNELS,
        default=fetch_sdk.DEFAULT_CHANNEL,
    )
    parser.add_argument(
        "--out",
        dest="out_dir",
        type=Path,
        default=Path("dist"),
        help="Directory for generated tarballs.",
    )
    parser.add_argument(
        "--sdk",
        dest="sdk_dir",
        type=Path,
        help="Write a combined SDK tree with include/, lib/, README.md, and LICENSE.sdk.md.",
    )
    parser.add_argument(
        "--zon",
        dest="update_zon",
        type=Path,
        help="Update generated SDK dependency entries in build.zig.zon.",
    )
    parser.add_argument(
        "--skip-release",
        action="store_true",
        help="Exit early when the resolved GitHub Release asset already exists.",
    )
    args = parser.parse_args()

    engine_sha, release, framework_sha = resolve_engine_sha(args.channel, args.sha)
    release_tag = f"{args.channel}-sdk-{engine_sha[:8]}"
    release_repository = os.environ.get("GITHUB_REPOSITORY", DEFAULT_RELEASE_REPOSITORY)
    base_url = release_url_base(release_repository, release_tag)
    github_output = (
        Path(os.environ["GITHUB_OUTPUT"]) if "GITHUB_OUTPUT" in os.environ else None
    )

    if args.skip_release and release_asset_exists(base_url):
        print(
            f"{PLATFORM_ASSET_NAMES['linux-x64']} already exists in {release_tag}; skipping package."
        )
        if github_output:
            write_github_output(
                github_output,
    ...
    if args.sdk_dir:
        fetch_sdk.reset_staged_sdk(args.sdk_dir)
    metadata_out = args.out_dir / "sdk-metadata.json"
    ...
    if args.sdk_dir:
        print(f"SDK directory: {args.sdk_dir}")
    print()
