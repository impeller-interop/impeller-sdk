...
def stage_platform_sdk(
    ...
    license_file = first_existing(
        extract_dir, ("LICENSE.sdk.md", "LICENSE", "LICENSE.md")
    )
    ...
    lib_dir = sdk_root / "lib" if flat_lib else sdk_root / "lib" / os_dir / arch_dir
    for pattern in (
        "libimpeller.so",
        "libimpeller.dylib",
        "impeller.dll",
...
def main() -> int:
    ...
    if args.sha:
        ...
        if not isinstance(framework_sha_value, str):
            raise RuntimeError(
                f"latest {args.channel} release does not contain a framework hash"
            )
    ...
    with tempfile.TemporaryDirectory(prefix="impeller_sdk_") as temp_dir:
        ...
        for platform in DEFAULT_PLATFORMS:
            ...
            copied = stage_platform_sdk(
                output, platform, sdk_root, args.force, temp_root
            )
